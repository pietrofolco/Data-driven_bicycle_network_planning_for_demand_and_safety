...
def greedy_triangulation(GT, GT_NOex, poipairs, D = 1000, existing_edge_length = 0,node_weights = None, existing_edges = None,poidist_and_alpha = [500,0.5]):
    """Greedy Triangulation (GT) of a graph GT with the existing abstract edges as edge set.
...
def poipairs_by_distance(G, pois, return_distances = False):
    ...
    for c, v in enumerate(indices):
        print('Get sequences of nodes and edges in shortest paths between all pairs of pois...')
        print(c+1,'/',len(indices))
        poi_node = []
        a = G.get_shortest_paths(v, indices[c:], weights = "weight", output = "vpath")
        for aa in a:
            poi_node.append((aa[0],aa[-1]))
...
def delaunay(GT, poipairs, D = 1, existing_edges = None,poidist_and_alpha = [500,0.5],length_bike=0,pairs=None,pois=None):
    """Delaunay Triangulation (DT) of a graph with the existing abstract edges as edge set.
    Distances between pairs of nodes are given by poipairs.
    ...
    # define delta and alpha
    poi_distance = poidist_and_alpha[0]
    alpha = poidist_and_alpha[1]
    ...
    for j,item in enumerate(pairs):
        ...
        if (poi0,poi1) not in existing_edges and (poi1,poi0) not in existing_edges:
                
            # for each link define count the number of accidents/trips per 1000 meters (if we simply count the number of
            # accidents/trips through each link, we advantage the longer links)
            #poipair_distance = dist(GT.vs.find(id = poi0), GT.vs.find(id = poi1))
            try:
                poipair_distance = poipairs_dict[str(poi0)+','+str(poi1)]
                acc_perKM = counter_dict[str(poi0)+','+str(poi1)][0]*1000/poipair_distance
                trips_perKM = counter_dict[str(poi0)+','+str(poi1)][1]*1000/poipair_distance
                w_acc = (poipair_distance + 1)/(1 + 9*acc_perKM/max_acc)
                w_trips = (poipair_distance + 1)/(1 + 9*trips_perKM/max_trips)
                # combine the contribution of accidents and trips data
                w01 = alpha*w_trips + (1-alpha)*w_acc
                GT.add_edge(GT.vs.find(id = poi0).index, GT.vs.find(id = poi1).index, weight = w01,length = poipair_distance)
                del poipairs_dict[str(poi0)+','+str(poi1)]
            except:
                try:
                    poipair_distance = poipairs_dict[str(poi1)+','+str(poi0)]
                    acc_perKM = counter_dict[str(poi1)+','+str(poi0)][0]*1000/poipair_distance
                    trips_perKM = counter_dict[str(poi1)+','+str(poi0)][1]*1000/poipair_distance
                    w_acc = (poipair_distance + 1)/(1 + 9*acc_perKM/max_acc)
                    w_trips = (poipair_distance + 1)/(1 + 9*trips_perKM/max_trips)
                    # combine the contribution of accidents and trips data
                    w01 = alpha*w_trips + (1-alpha)*w_acc
                    GT.add_edge(GT.vs.find(id = poi0).index, GT.vs.find(id = poi1).index, weight = w01,length = poipair_distance)
                    del poipairs_dict[str(poi1)+','+str(poi0)]
                except:
                    pass
            
   
    L = sum(GT.es['length']) - length_bike - D
    ...
    print('Length: ', edge_lengths(GT) - length_bike)
        
    
    return GT,BW


def delaunay_routing(G, pois, prune_quantiles = [1], prune_measure = "betweenness", existing = False, G_bike = None,poidist_and_alpha = [500,0.5],limited_abstract_edges = None):
    """
    Delaunay Triangulation (DT) of a graph G's node subset pois,
    then routing to connect the DT up to D km of investments.
    G is an ipgraph graph, pois is a list of node ids.
    
    Distance here is routing distance.
    """
    length_bike = sum(G_bike.es['weight']) # length of existing bike net
    ...
    for e in G.es:    
        ...
        for k in range(len(G_bike.es)):
            if e.attributes() == G_bike.es[k].attributes():
                ...
                G_temp.add_edge(e.source,e.target,weight=w,length=e['weight'],osmid = e['osmid'])
                existing_edges.append((e.source_vertex["id"], e.target_vertex["id"])) 
                break
    ...
    # create a list with the coordinates of seed points
    Xs = [G.vs.find(id = poi)['x'] for poi in pois]
    Ys = [G.vs.find(id = poi)['y'] for poi in pois]
    points = []
    ...
    tri = Delaunay(points)
    pairs = set()
    for j,item in enumerate(tri.simplices):
    ...
    GTs = []
    BWs = []
    for D in tqdm(Ds desc = "Delaunay triangulation", leave = False):
        ...
        GT_abstract,BW = delaunay(GT_abstract, poipairs, D, existing_edges,poidist_and_alpha,length_bike,pairs,pois)
        GT_abstracts.append(GT_abstract)
        BWs.append(BW)
